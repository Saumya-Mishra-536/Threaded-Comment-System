...
const LoginPage = ({ onAuth, authError }) => {
  ...
  const submit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await onAuth('login', form.username, form.password);
      navigate('/');
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-md">
      ...
      <form onSubmit={submit} className="space-y-3 bg-white p-4 rounded-md border border-gray-200">
        ...
        <button disabled={submitting} className="w-full bg-primary-blue text-white py-2 rounded hover:bg-blue-600 disabled:opacity-50">Login</button>
      </form>
    </div>
  );
};
